...
fn solve_part1(input: &str) -> usize {
    ...
    let n = num_ore_required(&ReactionPart::new(1, "FUEL".into()),
        &reaction_by_output,
        &mut waste);
    println!("waste remaining @ end: {:?}", waste);
    n
}

fn num_ore_required(target: &ReactionPart, reactions_list: &ReactionsList, waste: &mut Waste) -> usize {
    println!("wanting to get {:?}", target);
    if target.chemical == "ORE" {
        println!("it's just ore, so give em {} of it", target.parts);
        return target.parts
    }
    ...
    if num_pilfered_from_waste != 0 {
        println!("pilfered {} parts of {} to satisfy part of that", num_pilfered_from_waste, target.chemical);
    }

    let num_reactions = (num_parts_needed as f64 / reaction.output.parts as f64).ceil() as usize;
    println!("needed {} reactions to satisfy that, after pilfering", num_reactions);
    let wasted_parts = (num_reactions * reaction.output.parts) - num_parts_needed;
    println!("{} new parts of {} were wasted", wasted_parts, target.chemical);
...
impl Waste {
    ...
    fn pilfer(&mut self, chemical: &str, num_parts_needed: usize) -> usize {
        ...
        *num_parts_in_waste -= num_parts_to_take_from_waste;
        let pilfered = num_parts_to_take_from_waste;
        pilfered
    }
}
