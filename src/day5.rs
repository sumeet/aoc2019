use crate::day5::Instruction::{Add1, Multiply2, Input3, Output4, Halt99};
use crate::day5::ParameterMode::{PositionMode, ImmediateMode};
...
impl Instruction {
    fn parse(s: &str) -> Self {
        println!("parsing {} as instruction", s);
        let parsed = if s.ends_with("01") {
            let s = format!("{:0>5}", s);
            ...
            Add1(PositionMode, PositionMode, PositionMode)
        } else if s == "2" {
            Multiply2(PositionMode, PositionMode, PositionMode)
        } else if s == "3" {
            Input3
        } else if s == "4" {
            Output4
...
fn run_proggy(mut proggy: Vec<String>) -> String {
    ...
    loop {
        ...
        match instruction {
            ...
            Input3 => {
                let position = get_first_param(&proggy, current_pos, ImmediateMode) as usize;
                // this is the only input instruction...
                proggy[position] = "1".to_owned();
                current_pos += 2;
            },
            Output4 => {
                let param = get_first_param(&proggy, current_pos, PositionMode);
                println!("{}", param);
                current_pos += 2;
            },
            Halt99 => {
...
fn get_third_param(proggy: &[String], instruction_pos: usize, mode: ParameterMode) -> isize {
    if let ParameterMode::ImmediateMode = mode {
